      ...
      <div className={cn("relative bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-h-[90vh] overflow-hidden flex flex-col animate-slide-up", sizes[size])}>
        {title && (
          <div className="px-5 py-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
            ...
            <button onClick={onClose} className="text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-100 text-2xl leading-none">×</button>
          </div>
        )}
...
const MEDICAL_AVATARS = [
  ...
  "https://api.iconify.design/fluent-emoji/man-health-worker-light.svg",
  "https://api.iconify.design/fluent-emoji/woman-health-worker-light.svg",
  "https://api.iconify.design/fluent-emoji/man-health-worker-medium-dark.svg",
  "https://api.iconify.design/fluent-emoji/woman-health-worker-medium-dark.svg",
    ...
    <Modal 
      ...
      title={
        <div className="flex flex-col">
          <span>Select Medical Avatar</span>
          <span className="text-xs text-slate-400 mt-1 font-normal">Choose your specialty or uniform</span>
        </div>
      }
    >
      <div className="grid grid-cols-3 sm:grid-cols-4 gap-4">
        {MEDICAL_AVATARS.map((url, i) => (
          <button
            ...
            className="aspect-square rounded-xl bg-slate-100 dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 hover:border-teal-500 hover:bg-slate-200 dark:hover:bg-slate-700 transition-all flex items-center justify-center p-3 group relative overflow-hidden"
          >
